      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-[#071521] rounded-3xl border border-[#E5E7EB] dark:border-[#1E3A4D] shadow-2xl p-5 sm:p-6 space-y-5">
        ...
        <div className="flex items-center justify-between border-b border-[#E5E7EB] dark:border-[#1E3A4D] pb-4">
          <div>
            <div className="flex items-center space-x-2">
              <span className="text-[10px] font-mono font-bold text-[#0F766E] dark:text-[#14B8A6] uppercase bg-[#0F766E]/15 dark:bg-[#0F766E]/10 px-2 py-0.5 rounded flex items-center gap-1">
        ...
        {isSubmitted ? (
          <div className="py-12 text-center space-y-3">
            <div className="w-14 h-14 rounded-full bg-[#0F766E]/15 dark:bg-[#0F766E]/15 text-[#0F766E] dark:text-[#14B8A6] flex items-center justify-center mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 max-h-[72vh] overflow-y-auto pr-1">
            {/* Overall Experience */}
            <div className="space-y-2 p-3.5 rounded-2xl bg-[#F8FAFC] dark:bg-[#0B1F33] border border-[#E5E7EB] dark:border-[#1E3A4D]">
              ...
              <div className="flex items-center space-x-2 pt-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  ...
                  >
                    <Star className={`w-6 h-6 ${star <= rating ? 'fill-[#F4A261] text-[#F4A261]' : 'text-[#D1D5DB] dark:text-[#1E3A4D]'}`} />
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-[#6B7280] dark:text-[#94A3B8]">Infrastructure Scorecard</label>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                {/* Power */}
                <div className="p-3 rounded-xl bg-[#F8FAFC] dark:bg-[#0B1F33] border border-[#E5E7EB] dark:border-[#1E3A4D] space-y-1.5">
                    ...
                    {[1, 2, 3, 4, 5].map((s) => (
                      <button
                        ...
                        className={`h-2 flex-1 rounded-full transition-all cursor-pointer ${
                          s <= powerRating ? 'bg-[#0F766E]' : 'bg-[#E5E7EB] dark:bg-[#1E3A4D]'
                        }`}
                      />
                    ))}
                  </div>
                  ...
                  <div className="flex space-x-1">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <button
                        ...
                        className={`h-2 flex-1 rounded-full transition-all cursor-pointer ${
                          s <= internetRating ? 'bg-[#0F766E]' : 'bg-[#E5E7EB] dark:bg-[#1E3A4D]'
                        }`}
                      />
                    ))}
                  </div>
                  ...
                  <div className="flex space-x-1">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <button
                        ...
                        className={`h-2 flex-1 rounded-full transition-all cursor-pointer ${
                          s <= noiseRating ? 'bg-[#0F766E]' : 'bg-[#E5E7EB] dark:bg-[#1E3A4D]'
                        }`}
                      />
                    ))}
                  </div>
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-[#6B7280] dark:text-[#94A3B8]">Verified On-Site Features</label>
                  ...
                  return (
                    <button
                      ...
                      className={`p-2.5 rounded-xl border text-left text-[11px] font-medium flex items-center space-x-2 transition-all cursor-pointer ${
                        isSelected
                          ? 'bg-[#0F766E]/15 dark:bg-[#0F766E]/15 border-[#0F766E]/40 text-[#111827] dark:text-[#F2F2F2]'
                          : 'bg-white dark:bg-[#0B1F33] border-[#E5E7EB] dark:border-[#1E3A4D] text-[#6B7280] dark:text-[#718079] hover:text-[#111827] dark:hover:text-[#94A3B8]'
                      }`}
                    >
                      <div className={`w-4 h-4 rounded flex items-center justify-center shrink-0 ${
                        isSelected ? 'bg-[#0F766E] text-white' : 'bg-[#E5E7EB] dark:bg-[#1E3A4D]'
                      }`}>
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                rows={3}
                required
                className="w-full p-3 rounded-xl bg-[#F8FAFC] dark:bg-[#0B1F33] border border-[#E5E7EB] dark:border-[#1E3A4D] text-xs text-[#111827] dark:text-[#F2F2F2] placeholder-[#9CA3AF] dark:placeholder-[#718079] focus:outline-none focus:border-[#0F766E]"
              />
            </div>
            ...
            <div className="pt-2 flex items-center justify-end space-x-3">
              <button
                type="button"
                onClick={() => setIsWriteReviewOpen(false)}
                className="px-4 py-2.5 rounded-xl text-xs font-semibold text-[#6B7280] dark:text-[#94A3B8] hover:text-[#111827] dark:hover:text-[#F2F2F2] hover:bg-[#F1F5F9] dark:hover:bg-[#0B1F33] cursor-pointer"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="px-5 py-2.5 rounded-xl bg-[#0F766E] hover:bg-[#14B8A6] text-white font-bold text-xs shadow-md transition-all active:scale-95 cursor-pointer"
              >
